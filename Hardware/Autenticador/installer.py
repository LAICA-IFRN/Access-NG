"""
Access-NG - Instalador do esquema boot.py/main.py/accessng/ para um
Caronte ESP32-C3 com MicroPython recém-gravado (sem nenhum arquivo de
aplicação ainda) - ou para recuperar um dispositivo travado, apagando
...
HTTP puro (mesma rota /ota/<filepath> que o OTA normal já usa).

--- USO -----------------------------------------------------------------

  1. Grave o firmware MicroPython no ESP32-C3 normalmente (esptool),
     se ainda não estiver gravado.
  ...
  3. Copie e rode com mpremote (troque /dev/ttyUSB0 pela porta certa):

       mpremote connect /dev/ttyUSB0 cp installer.py :installer.py
       mpremote connect /dev/ttyUSB0 run installer.py
  ...
  4. Ao terminar, o dispositivo reinicia sozinho. Se CONFIG tiver pelo
     ...
     no modo recovery (AP "AccessNG-Caronte-XXXX" em 192.168.4.1) depois
     de algumas tentativas - complete por lá.

--- ERASE_ANTES = True ---------------------------------------------------

  Útil para recuperar um dispositivo que ficou preso num estado ruim
  (ex.: boot.py/main.py corrompidos, ou o crash-loop do watchdog no
  portal de provisionamento visto em campo antes da correção em
  accessng/provisioning.py) - apaga os arquivos da aplicação (boot.py,
  main.py, device_defaults.py, accessng/, umqtt/, boot_state.json)
  antes de reinstalar do zero. NÃO apaga config.json nem tags.json
  (preserva a configuração/whitelist já gravada, se você não quiser
  perdê-las - mas CONFIG abaixo só é escrito em config.json se ele
  ainda não existir, então tanto faz).
"""
...
_FILES = [
    ...
    ("Hardware/Autenticador/boot.py", "/boot.py"),
    ("Hardware/Autenticador/main.py", "/main.py"),
]

_ERASE_PATHS = [
    "/boot.py", "/main.py", "/device_defaults.py", "/boot_state.json",
    "/main.bak", "/main.py.new",
...
def _http_download(repo_path, dest_path, timeout=20):
    """GET HTTP simples (sem TLS - mesmo motivo de sempre neste projeto:
    o handshake RSA estoura a memória disponível no ESP32-C3) - grava
    direto em dest_path por streaming, mesma lógica já testada em
    accessng.ota.http_request(), reproduzida aqui de forma autocontida
    porque accessng ainda não existe no dispositivo neste ponto."""
    sock = None
    try:
...
def run():
    print("\n" + "=" * 56)
    print("  Access-NG - Instalador (Caronte ESP32-C3)")
    print("=" * 56)
