# Cerberos+Caronte BitDogLab V6 / Pico W - supervisor de boot
#
...
#
# Instalado no dispositivo como boot.py (o sufixo _bitdoglab é só pra
# conviver com as outras variantes de Cerberos dentro deste diretório).
#
# network.WLAN(STA_IF)/AP_IF é a mesma API do ESP32 no Pico W (cyw43 é
# encapsulado, não é uma API separada) - já usado sem diferença pelo
# firmware original deste dispositivo. accessng.wifi/recovery/
# provisioning não têm nenhum branch específico de RP2040.

DEVICE_TYPE = "BitDogLab"

import machine
from device_defaults import DEFAULTS, SENSITIVE_KEYS
from accessng import config, wifi, recovery, ota, watchdog

# 0) Watchdog de hardware - ver accessng/watchdog.py. Cap de ~8.3s no
#    RP2040 é o motivo do timeout conservador de 8000ms lá dentro.
watchdog.arm()
