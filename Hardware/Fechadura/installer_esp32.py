...
Cerberos ESP32 (enxuto) com MicroPython recém-gravado (sem nenhum arquivo
...
padrão dos instaladores do Caronte e do FECHO.

--- USO -----------------------------------------------------------------

  1. Grave o firmware MicroPython no ESP32 normalmente (esptool), se
     ainda não estiver gravado.
  ...
  3. Copie e rode com mpremote (troque /dev/ttyUSB0 pela porta certa):
       ...
       mpremote connect /dev/ttyUSB0 cp installer_esp32.py :installer.py
  ...
  4. Ao terminar, o dispositivo reinicia sozinho. Se CONFIG tiver pelo
     ...
     no modo recovery (AP "AccessNG-Cerberos-XXXX" em 192.168.4.1) depois
...
--- ERASE_ANTES = True ---------------------------------------------------
  ...
  (boot.py, main.py, device_defaults.py, accessng/, umqtt/,
...
_FILES = [
    ...
    ("Hardware/Fechadura/boot_esp32.py", "/boot.py"),
    ("Hardware/Fechadura/main_esp32.py", "/main.py"),
]
...
def _http_download(repo_path, dest_path, timeout=20):
    ...
    disponível no ESP32) - grava direto em dest_path por streaming, mesma
    lógica já testada em accessng.ota.http_request(), reproduzida aqui de
    forma autocontida porque accessng ainda não existe no dispositivo
    neste ponto."""
    sock = None
    try:
...
def run():
    print("\n" + "=" * 56)
    print("  Access-NG - Instalador (Cerberos ESP32 / enxuto)")
    print("=" * 56)
