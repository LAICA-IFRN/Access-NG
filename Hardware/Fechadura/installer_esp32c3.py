...
Cerberos ESP32-C3 (FECHO) com MicroPython recém-gravado (sem nenhum
arquivo de aplicação ainda) - ou para recuperar um dispositivo travado,
apagando tudo primeiro (ver ERASE_ANTES abaixo).
...
padrão do instalador do Caronte (Hardware/Autenticador/installer.py).
...
--- USO -----------------------------------------------------------------
  ...
  3. Copie e rode com mpremote (troque /dev/ttyUSB0 pela porta certa):
       ...
       mpremote connect /dev/ttyUSB0 cp installer_esp32c3.py :installer.py
  ...
  4. Ao terminar, o dispositivo reinicia sozinho. Se CONFIG tiver pelo
     ...
     no modo recovery (AP "AccessNG-FECHO-XXXX" em 192.168.4.1) depois
...
--- ERASE_ANTES = True ---------------------------------------------------
  ...
  main.py, device_defaults.py, accessng/, umqtt/, sh1106.py,
  boot_state.json) antes de reinstalar do zero. NÃO apaga config.json
  (preserva a configuração já gravada, se você não quiser perdê-la -
  mas CONFIG abaixo só é escrito em config.json se ele ainda não
  existir, então tanto faz).
"""
...
_FILES = [
    ...
    ("Hardware/Fechadura/boot_esp32c3.py", "/boot.py"),
    ("Hardware/Fechadura/main_esp32c3.py", "/main.py"),
]

_ERASE_PATHS = [
    "/boot.py", "/main.py", "/device_defaults.py", "/boot_state.json",
    "/main.bak", "/main.py.new", "/sh1106.py",
...
def _http_download(repo_path, dest_path, timeout=20):
    """GET HTTP simples (sem TLS - o handshake RSA estoura a memória
    disponível no ESP32-C3) - grava direto em dest_path por streaming,
    mesma lógica já testada em accessng.ota.http_request(), reproduzida
    aqui de forma autocontida porque accessng ainda não existe no
    dispositivo neste ponto."""
    sock = None
    try:
...
def run():
    print("\n" + "=" * 56)
    print("  Access-NG - Instalador (Cerberos ESP32-C3 / FECHO)")
    print("=" * 56)
